...
use futures::{Future, SinkExt, StreamExt, future};
use url::Url;
...
async fn handle_request(request: IncomingRequest, response_out: ResponseOutparam) {
    ...
    eprintln!("[COMPONENT] Method: {:?}", request.method());
    eprintln!("[COMPONENT] Path: {:?}", request.path_with_query());
    eprintln!("[COMPONENT] Authority: {:?}", request.authority());
    eprintln!("[COMPONENT] Headers: {:?}", headers);

    assert!(request.authority().is_some());

    match (request.method(), request.path_with_query().as_deref()) {
        (Method::Post, Some("/gemini-proxy")) => {
            eprintln!("[COMPONENT] Matched /gemini-proxy route, proxying to Gemini API");
            // Pipe the request body to Gemini API and stream the response back to the client.

            match gemini_proxy(request).await {
                Ok(response) => {
                    eprintln!("[COMPONENT] Got response from Gemini API, starting to stream");
                ...
                Err(e) => {
                    eprintln!("[COMPONENT] Error sending outgoing request to Gemini API: {e}");
